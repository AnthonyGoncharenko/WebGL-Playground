class Drawable {
    ...
    initializeWireFrameBuffer(indices) {
        this.vertexPositions = indices;

        this.elementArrayBuffer = gl.createBuffer();
        gl.bindBuffer(gl.ELEMENT_ARRAY_BUFFER, this.elementArrayBuffer);
        gl.bufferData(gl.ELEMENT_ARRAY_BUFFER, new Uint8Array(this.vertexPositions), gl.STATIC_DRAW);
        this.aPosition = gl.getAttribLocation(this.shaderProgram, "aPosition");
        return this;
    }
    ...
    bindModelMatrixUniform() {
        gl.uniformMatrix4fv(this.uModelMatrix, false, flatten(this.getModelMatrix()));
        return this;
    }
    ...
    drawAndDestructWireFrame(drawStyle) {
        gl.enableVertexAttribArray(this.aPosition);
        gl.drawElements(drawStyle, this.vertexPositions.length, gl.UNSIGNED_BYTE, 0);
        gl.disableVertexAttribArray(this.aPosition);
    }
    ...
    drawUniform(dimension, drawStyle) {
        this
            .useProgram()
            .bindPositionBuffer(dimension)
            .bindUniformColor()
    ...
    drawPyramid(dimension, drawStyle) {
        this
            .useProgram()
            .bindPositionBuffer(dimension)
            .bindColorBuffer()
